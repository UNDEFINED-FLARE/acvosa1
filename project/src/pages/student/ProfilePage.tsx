...
import { Mail, Building2, CalendarDays, Award, Clock, CheckSquare, LogOut } from 'lucide-react';

export function ProfilePage() {
  const { attendanceRecords, reservations, impact } = useApp();
  const { user, signOut } = useAuth();
  const { navigate } = useNav();

  const userFullName = user ? `${user.firstName} ${user.lastName}`.trim() : 'Student';
  const userInitials = user ? `${user.firstName[0] ?? ''}${user.lastName[0] ?? ''}`.toUpperCase() : 'S';
  const userEmail = user?.email ?? '';
  const userFaculty = user?.faculty ?? '';
  const userStudentNumber = user?.studentNumber ?? '';
  const userJoined = user?.createdAt ? new Date(user.createdAt).toLocaleDateString('en-ZA', { year: 'numeric', month: 'short' }) : '2024';
  ...
  return (
    <PageContainer className="pb-28 lg:pb-10">
      ...
      <Card className="mt-6 p-0 overflow-hidden" hover={false}>
        ...
        <div className="px-5 sm:px-7 pb-6">
          <div className="flex items-end justify-between -mt-10">
            <Avatar initials={userInitials} size="lg" className="ring-4 ring-ink-white !w-20 !h-20 !text-2xl" />
            <Badge tone="dark" dot>Active Member</Badge>
          </div>
          <h2 className="text-xl font-bold text-ink-charcoal tracking-tight mt-4">{userFullName}</h2>
          <p className="text-sm text-ink-dark-grey/65 mt-1 tracking-tight">{userFaculty}</p>
          ...
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div className="flex items-center gap-2.5 text-ink-dark-grey/75">
              ...
              <span className="tracking-tight truncate">{userEmail}</span>
            </div>
            {userStudentNumber && (
              <div className="flex items-center gap-2.5 text-ink-dark-grey/75">
                <span className="text-2xs text-ink-dark-grey/45 uppercase tracking-wider">Student No.</span>
                <span className="font-medium tracking-tight tabular-nums">{userStudentNumber}</span>
              </div>
            )}
            <div className="flex items-center gap-2.5 text-ink-dark-grey/75">
              <CalendarDays size={15} className="text-ink-dark-grey/45" />
              <span className="tracking-tight">Joined {userJoined}</span>
            </div>
          </div>
        </div>
      </Card>
      ...
      <div className="mt-6 flex items-center gap-3">
        <button onClick={() => signOut().then(() => navigate('login'))} className="flex items-center gap-2 text-sm text-ink-dark-grey/60 hover:text-ink-charcoal transition-colors tracking-tight">
