    ...
    <header className="sticky top-0 z-30 bg-ink-off-white/85 backdrop-blur-lg border-b border-ink-light-grey">
      <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button onClick={() => nav(admin ? 'admin-settings' : 'profile')} className="flex items-center gap-2.5 pl-1">
            <Avatar initials={initials(user.name)} size="sm" />
            <div className="hidden sm:flex flex-col leading-tight">
              <span className="text-sm font-medium text-ink-charcoal tracking-tight max-w-[140px] truncate">{user.name}</span>
